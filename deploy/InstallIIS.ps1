...
Node $nodeName
  {
	...
	Script DownloadCoreSDK
    {
        TestScript = {
            Test-Path "C:\WindowsAzure\dotnet-sdk-2.1.202-win-x64.exe"
        }
        SetScript ={
            $source = "https://download.microsoft.com/download/f/c/1/fc16c864-b374-4668-83a2-f9f880928b2d/dotnet-sdk-2.1.202-win-x64.exe"
            $dest = "C:\WindowsAzure\dotnet-sdk-2.1.202-win-x64.exe"
            Invoke-WebRequest $source -OutFile $dest
        }
        GetScript = {@{Result = "DownloadCoreSDK"}}
        DependsOn = "[WindowsFeature]WebServerRole"
    }
	
	Package InstallCoreSDK
    {
        Ensure = "Present"  
        Path  = "C:\WindowsAzure\dotnet-sdk-2.1.202-win-x64.exe"
        Name = "Microsoft .NET Core SDK - 2.1.202 (x64)"
        ProductId = "{0EC8A4AE-29FE-43B9-90C8-4D8D7D62393C}"
        Arguments = "/quiet"
        DependsOn = "[Script]DownloadCoreSDK"
    }
	
	Script DownloadCoreHosting
    {
        TestScript = {
            Test-Path "C:\WindowsAzure\DotNetCore.2.0.9-WindowsHosting.exe"
        }
        SetScript ={
            ...
            $dest = "C:\WindowsAzure\DotNetCore.2.0.9-WindowsHosting.exe"
            Invoke-WebRequest $source -OutFile $dest
        }
        GetScript = {@{Result = "DownloadCoreHosting"}}
	}
	
	Package InstallCoreHosting
    {
        Ensure = "Present"  
        Path  = "C:\WindowsAzure\DotNetCore.2.0.9-WindowsHosting.exe"
        Name = "Microsoft .NET Core Host - 2.0.9 (x64)"
        ProductId = "{0B28A1DB-A543-45FB-8767-DF71EE5A9323}"
        Arguments = "/quiet"
        DependsOn = "[Script]DownloadCoreHosting"
    }
